;Write Boot 1.0 Copyright (c) 1999 by Marcelo Gornstein.
;This program is under the GNU/GPL version 2 license.
;Write the first 512 bytes from a file to the first sector on
;the first floppy drive.
;You may pass the the input file name in the command tail.
;If there are no parameters, the default input file will be
...
;-------------------------------------------------
OpenFile:
        mov ax, 3D02h      ;DOS open file service.
        lea dx, FILE_NAME  ;DX points to the filename.
        int 21h            ;Do it.
        jc Error           ;Oops.
        push ax            ;We get the new handle in AX. Save it.
;----------------------------------------------------------------------
;----- ReadtoBuffer: Reads the contents of the file into the      -----
;----- allocated segment.
;----------------------------------------------------------------------
ReadtoBuffer:
        mov ax, 3F00h   ;DOS read from file with handle service.
        ...
        jc Error        ;Error?
;-------------------------------------------------------------------
;----- CloseFile: after filling the buffer, we close the file. -----
...
;-----------------------------------------------------------------
;----- WriteSector: Using BIOS int 26h (Absolute disk write) -----
;----- we write to sector 0 from drive 0 (A)                 -----
;-----------------------------------------------------------------
WriteSector:
        xor ax, ax      ;Clear AX.
...
COPYRIGHT db 'Write Boot 1.0 Copyright (c) 1999 by Marcelo Gornstein.', 0a, 0d
